        ...
        private void CarTextBox_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lCarID > 0)//如果存在该车辆
                {
                    ...
                    DataTable dtInBill = ExecuteSQL.CallView(125, "SaleCarInBillID", "SaleCarOutBillID is null and IsCancel = 0", "");
                    ...
                    DataTable dtInReturnBill = ExecuteSQL.CallView(137, "", "ReturnStatus=0 and CarID="+ lCarID.ToString(), "");
                    if (dtInReturnBill.Rows.Count > 0)
                    {
                        DataRow drReturnBill = dtInReturnBill.Rows[0];

                        long lSaleCarInBillID = LBConverter.ToInt64(drReturnBill["SaleCarInBillID"]);
                    ...
                    else
                    {
                        ...
                        DataTable dtLastOutBill = ExecuteSQL.CallView(125,
                            @"SaleCarInBillID,SaleCarOutBillID,SaleCarOutBillCode,SaleCarInBillCode,CarID, CarNum, ItemID,ItemName,
                            CustomerID, CustomerName,CarTare,BillDateOut,TotalWeight,SuttleWeight",
                        "IsCancel = 0 and SaleCarOutBillID is not null and BillStatus=2 and BillDateOut>getdate()-1 and CarID="+lCarID.ToString(), "SaleCarOutBillID desc");

                        if (dtLastOutBill.Rows.Count == 0)
                        {
                            throw new Exception("该车辆最近一天内没有出场记录，无法退货！");
                        }
                        else
                        {
