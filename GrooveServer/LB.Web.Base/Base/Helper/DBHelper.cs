	...
	public class DBHelper
	{
		...
		public static int ExecuteNonQuery(FactoryArgs args, CommandType cmdType, string cmdText, LBDbParameterCollection cmdParms, bool bNeedTrans )
		{
						...
						try
						{
                            ...
                            using ( DbTransaction trans = conn.BeginTransaction( level ) )
							{
								using( DbCommand cmd = Provider.CreateCommand() )
								{
									...
									try
									{
										iResult = _ExecuteNonQuery(args, cmd, conn, trans, cmdType, cmdText, cmdParms );
										trans.Commit();
									}
									catch( Exception ex )
									{
				...
				else
				{
					...
					using( DbConnection conn = Provider.CreateConnection( args.DBName ) )
					{
						try
						{
							using( DbCommand cmd = Provider.CreateCommand() )
							{
								return _ExecuteNonQuery(args, cmd, conn, null, cmdType, cmdText, cmdParms );
							}
						}
						finally
						{
			...
			else if( args.DbTrans != null )
			{
				...
				using( DbCommand cmd = Provider.CreateCommand() )
				{
					return _ExecuteNonQuery(args, cmd, args.DbConnection, args.DbTrans, cmdType, cmdText, cmdParms );
				}
			...
			else if( bNeedTrans )
			{
                ...
                using ( DbTransaction trans = args.DbConnection.BeginTransaction( level ) )
				{
					using( DbCommand cmd = Provider.CreateCommand() )
					{
						...
						try
						{
							iResult = _ExecuteNonQuery(args, cmd, args.DbConnection, trans, cmdType, cmdText, cmdParms );
							trans.Commit();
						}
						catch( Exception ex )
						{
			...
			else
			{
				...
				using( DbCommand cmd = Provider.CreateCommand() )
				{
					return _ExecuteNonQuery(args, cmd, args.DbConnection, null, cmdType, cmdText, cmdParms );
				}
                            ...
                            using ( DbTransaction trans = conn.BeginTransaction( level ) )
							{
								using( DbCommand cmd = Provider.CreateCommand() )
								{
									try
									{
										...
										// 取输出参数值
										GetParameterOut( cmdParms, cmd.Parameters, dtResult);
        ...
        private static void SQLConverter(ref string strSQL)
        {
            strSQL = strSQL.Replace("isnull", "ifnull").Replace("dbo.", "");
        }
